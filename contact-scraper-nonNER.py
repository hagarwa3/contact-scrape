...
for url in url2:
    ...
    m = m.split("\n")
    m[:] = [x.decode('utf-8').strip() for x in m if x.decode('utf-8').strip() != '']
    m[:] = [x.replace(u'&nbsp', u' ') for x in m]
    m[:] = [x.replace(u'\xa0', u' ') for x in m]
    m[:] = [x.replace(u'0xa0', u' ') for x in m]
